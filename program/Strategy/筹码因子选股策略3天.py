...
def timing(equity):
    signal = tracking_signal(equity.copy(),[10, 0.10])
    return signal


def tracking_signal(df,para):
    # ===策略参数
    s = int(para[0])  # 期周数
    p = para[1]  # 下跌阈值

    df['max_equity'] = df['equity_curve'].rolling(s, min_periods=1).max()
    df['mdd'] = 1 - df['equity_curve'] / df['max_equity']

    df.loc[df['mdd'] < p, 'signal'] = 1
